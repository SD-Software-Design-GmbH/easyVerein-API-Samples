...
for nested_member_request, nested_contact_details_request in zip(nested_member_requests, nested_contact_details_requests):
    response[0] = requests.get('https://easyverein.com/api/v2.0/member?query={query}'.format(query=nested_member_request), headers=header)
    response[1] = requests.get('https://easyverein.com/api/v2.0/contact-details?query={query}'.format(query=nested_contact_details_request), headers=header)
