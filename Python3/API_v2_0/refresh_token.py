...
response = requests.get('https://easyverein.com/api/v2.0/member/', headers=header)

token_refresh_needed = response.headers.get('tokenRefreshNeeded')

if token_refresh_needed:
    response = requests.get('https://easyverein.com/api/v2.0/refresh-token/', headers=header)
